...
def get_current_user(access_token: Optional[str] = Cookie(None), db: Session = Depends(get_db)) -> User | None:
    ...
    if user.email == settings.DEFAULT_USER_EMAIL and getattr(user, "role", None) != "admin":
        user.role = "admin"
        try:
            db.commit()
        except Exception:
...
def require_admin_user(current_user: User = Depends(get_current_user)) -> User:
    ...
    is_admin = getattr(current_user, "role", "user") == "admin" or current_user.email == settings.DEFAULT_USER_EMAIL
    if not is_admin:
...
def create_user(email: str, password: str, db: Session, role: str = "user") -> User:
