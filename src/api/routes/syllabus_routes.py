...
from api.schemas.syllabus_run_schemas import (
    ...
    RerunSyllabusRequest,
    StartSyllabusRunRequest,
...
async def start_syllabus_run(
    course_id: str,
    req: StartSyllabusRunRequest | None = None,
    current_user: User = Depends(get_current_user),
    db: Session = Depends(get_db)
) -> StartSyllabusRunResponse:
    """
    Start a syllabus generation run for the course.
    Optionally accepts { provider, model } to customize which LLM plans the syllabus.
    ...
    try:
        ...
        run_id = syllabus_service.start_run(course_id, user_id, provider=provider, model=model)
        run = syllabus_service.get_run(run_id, user_id)
        snap = run.state_snapshot if run and isinstance(run.state_snapshot, dict) else {}
        resolved_provider = snap.get("provider")
        resolved_model = snap.get("inference_model")
        agent_name = snap.get("agent") or "TutorAgent"
        initial = {
            "stage": "generate_concepts",
            "state": snap,
            "done": False,
            "agent": agent_name,
...
@syllabus_routes.post("/courses/{course_id}/syllabus/rerun", response_model=StartSyllabusRunResponse)
async def rerun_syllabus_run(
    course_id: str,
    req: RerunSyllabusRequest | None = None,
    current_user: User = Depends(get_current_user),
    db: Session = Depends(get_db),
) -> StartSyllabusRunResponse:
    """
    Archive existing runs, reset confirmation, and start a fresh syllabus run with specified agent and model.
    """
    assert current_user is not None
    user_id = get_db_user_id(current_user.email, db)
    course = db.query(Course).filter(Course.id == course_id, Course.user_id == user_id).first()
    ...
    try:
        ...
        provider = req.provider if req else None
        model = req.model if req else None
        agent = req.agent if req else "TutorAgent"
        new_run_id = syllabus_service.rerun_for_course(
            course_id=course_id,
            user_id=user_id,
            provider=provider,
            model=model,
        ...
        run = syllabus_service.get_run(new_run_id, user_id)
        snap = run.state_snapshot if run and isinstance(run.state_snapshot, dict) else {}
        resolved_provider = snap.get("provider")
        resolved_model = snap.get("inference_model")
        agent_name = snap.get("agent") or agent or "TutorAgent"
        initial = {
            "stage": "planning",
            "state": snap,
            "done": False,
        ...
        await broadcast_syllabus_state(new_run_id, initial)
        return StartSyllabusRunResponse(run_id=new_run_id)
    except ValueError as e:
...
async def get_syllabus_run(
    ...
    snap = run.state_snapshot if isinstance(run.state_snapshot, dict) else {}
    provider = snap.get("provider")
    model = snap.get("inference_model")
    if not provider or not model:
        provider, model = syllabus_service._resolve_model_config(user_id, provider, model)
    agent_name = snap.get("agent") or "TutorAgent"
...
@syllabus_routes.post("/syllabus/runs/{run_id}/auto-run", response_model=AutoRunResponse)
async def auto_run_syllabus(
    run_id: str,
    max_steps: int = Query(30, description="Max execution steps to prevent infinite loops"),
    current_user: User = Depends(get_current_user),
    db: Session = Depends(get_db),
) -> AutoRunResponse:
    """
    Execute syllabus generation loop automatically until finished or max_steps reached.
    """
    assert current_user is not None
    user_id = get_db_user_id(current_user.email, db)
    syllabus_service = SyllabusService(db)
    res = await syllabus_service.auto_run(run_id, user_id, max_steps=max_steps)
    if not res.get("success"):
        raise HTTPException(status_code=400, detail=res.get("error", "Auto-run failed"))

    payload = {
        "stage": res.get("stage", "finalized"),
        "state": res.get("state", {}),
        "done": res.get("done", True),
        "agent": res.get("agent"),
        "provider": res.get("provider"),
        "inference_model": res.get("inference_model"),
    }
    await broadcast_syllabus_state(run_id, payload)
    return AutoRunResponse(
        ...
        status=res["status"],
        stage=res.get("stage"),
        steps_executed=res.get("steps_executed", 0),
        done=res.get("done", True),
        modules_count=res.get("modules_count", 0),
        agent=res.get("agent"),
        provider=res.get("provider"),
        inference_model=res.get("inference_model"),
        error=res.get("error"),
...
async def syllabus_run_websocket(
    ...
    try:
        ...
        inference_model = state.get("inference_model")
        agent_name = state.get("agent") or "TutorAgent"
        if not provider or not inference_model:
            provider, inference_model = syllabus_service._resolve_model_config(user_id, provider, inference_model)
