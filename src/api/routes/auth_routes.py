...
from api.schemas.user_schemas import User, UserProfileResponse 
...
@auth_routes.get("/me", response_model=UserProfileResponse)
def get_current_user_info(current_user = Depends(get_current_user)) -> UserProfileResponse:
    """Protected route to test authentication - returns current user info."""
    from api.settings import settings
    role = getattr(current_user, "role", "user")
    is_admin = role == "admin" or current_user.email == settings.DEFAULT_USER_EMAIL
