 ...
 */
function findFormContext(formHtml, htmlContent, formId) {
    const context = {};
 ...
 */
function generateFormSemanticDescription(inputs, buttons) {
    ...
    const allLabels = inputs.map(i => (i.metadata.label || i.attributes.placeholder || i.attributes.name || '').toLowerCase()).join(' ');
    let inferredPurpose = '';
 ...
 */
function extractComponents(htmlContent, textContent) {
    const components = [];
    ...
    while ((formMatch = formRegex.exec(htmlContent)) !== null) {
        ...
        const formComponent = {
            ...
            metadata: {
                isInteractive: true,
                children: inputs.map(i => i.id).concat(formButtons.map(b => b.id)),
    ...
    event_logger_1.eventLogger.info('Component Extraction', `Extracted ${components.length} components (${components.filter(c => c.type === 'form').length} forms, ${components.filter(c => c.type === 'button').length} buttons, ${components.filter(c => c.type === 'table').length} tables)`);
    return components;
}
// Helper functions
...
function findLabelForInput(inputId, inputName, htmlContent) {
