...
function chunkContent(pageContent, components = []) {
    event_logger_1.eventLogger.info('Chunking', `Starting content chunking for page: ${pageContent.title}`);
    const chunks = [];
    const { extractedText, structure } = pageContent;
    // Log section info for debugging
    if (structure.sections.length > 0) {
        event_logger_1.eventLogger.info('Chunking', `Found ${structure.sections.length} sections`);
        structure.sections.forEach((section, idx) => {
            ...
            event_logger_1.eventLogger.debug('Chunking', `Section ${idx + 1}: "${section.heading || 'No heading'}" - ${contentLength} chars, ${wordCount} words`);
        });
    }
    // Strategy 1: Component-based chunking (preferred for interactive elements)
    if (components.length > 0) {
        event_logger_1.eventLogger.info('Chunking', `Found ${components.length} components, creating component-aware chunks`);
    ...
    if (structure.sections.length > 0) {
        ...
        sectionChunks.forEach(sectionChunk => {
            ...
            const hasOverlap = components.some(comp => sectionChunk.content.includes(comp.textContent) ||
    ...
    chunks.forEach((chunk, idx) => {
        ...
        if (contentLength === 0) {
            event_logger_1.eventLogger.warning('Chunking', `Chunk ${idx + 1} has no content! Heading: "${chunk.metadata.heading || 'No heading'}"`);
        }
            ...
            event_logger_1.eventLogger.debug('Chunking', `Component chunk ${idx + 1} (${chunk.componentType}): ${contentLength} chars - complete element preserved`);
        }
        else if (contentLength > MAX_PARAGRAPH_CHARS) {
            // Warn if a single paragraph exceeds MAX_PARAGRAPH_CHARS (should have been split)
            event_logger_1.eventLogger.warning('Chunking', `Text chunk ${idx + 1} is very large: ${contentLength} chars (max ${MAX_PARAGRAPH_CHARS} for single paragraph). Heading: "${chunk.metadata.heading || 'No heading'}"`);
        }
            ...
            event_logger_1.eventLogger.debug('Chunking', `Text chunk ${idx + 1}: ${contentLength} chars - complete paragraph preserved (exceeds preferred ${MAX_CHUNK_CHARS} but within ${MAX_PARAGRAPH_CHARS} limit)`);
        }
    });
    ...
    event_logger_1.eventLogger.success('Chunking', `Created ${finalChunks.length} chunks from page content`);
    return finalChunks;
}
function chunkBySections(text, sections) {
    ...
    for (const section of sections) {
        ...
        if (wordCount === 0) {
            event_logger_1.eventLogger.warning('Chunking', `Skipping section "${section.heading || 'No heading'}" - no content`);
            continue;
        }
...
function splitLargeParagraph(paragraph, maxChars, sentenceOverlap = 0) {
...
function splitLargeSection(text, section) {
    const chunks = [];
    ...
    for (const paragraph of paragraphs) {
        ...
        if (paragraph.length > MAX_PARAGRAPH_CHARS) {
            ...
            event_logger_1.eventLogger.info('Chunking', `Splitting very large paragraph (${paragraph.length} chars) at sentence boundaries...`);
            const paragraphPieces = splitLargeParagraph(paragraph, MAX_CHUNK_CHARS, SENTENCE_OVERLAP);
            event_logger_1.eventLogger.info('Chunking', `Split into ${paragraphPieces.length} chunks with sentence overlap`);
...
function getOverlapText(text, wordCount) {
...
function extractStructure(htmlContent, textContent) {
    const sections = [];
    const headings = [];
    ...
    while ((match = headingRegex.exec(htmlContent)) !== null) {
        ...
        if (position !== -1) {
            ...
            if (currentSection) {
                ...
                currentSection.content = textContent.substring(prevHeadingEnd, position).trim();
    ...
    if (currentSection) {
        ...
        currentSection.content = textContent.substring(headingEnd, textContent.length).trim();
        sections.push(currentSection);
    }
 ...
 */
function chunkByComponents(text, sections, components) {
    const chunks = [];
    ...
    forms.forEach(form => {
        ...
        const nestedInputChunks = formInputs.map(input => ({
            id: (0, uuid_1.v4)(),
        ...
        const nestedButtonChunks = formButtons.map(button => ({
            id: (0, uuid_1.v4)(),
