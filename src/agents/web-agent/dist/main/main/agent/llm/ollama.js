...
exports.ensureModelLoaded = ensureModelLoaded;
exports.generateAnswer = generateAnswer;
exports.generateAnswerFromPrompt = generateAnswerFromPrompt;
exports.buildQAPrompt = buildQAPrompt;
exports.generateAnswerWithTools = generateAnswerWithTools;
...
async function checkOllamaConnection() {
    try {
        ...
        event_logger_1.eventLogger.info('Ollama', `Checking Ollama connection... Base URL: ${OLLAMA_BASE_URL}`);
        event_logger_1.eventLogger.debug('Ollama', `Will try URLs: ${urls.join(', ')}`);
        for (const url of urls) {
            try {
                event_logger_1.eventLogger.debug('Ollama', `Attempting connection to ${url}...`);
                ...
                event_logger_1.eventLogger.debug('Ollama', `Response from ${url}: status=${response.status}, duration=${duration}ms`);
                if (response.status === 200) {
                    event_logger_1.eventLogger.success('Ollama', `Connection successful at ${url} (${duration}ms)`);
                    return true;
                }
                else {
                    event_logger_1.eventLogger.warning('Ollama', `Unexpected status ${response.status} from ${url}`);
                }
            }
            catch (err) {
                const errorDetails = {
                ...
                event_logger_1.eventLogger.debug('Ollama', `${url} failed:`, JSON.stringify(errorDetails, null, 2));
                // Try next URL
                if (err.code === 'ECONNREFUSED' || err.code === 'ENOTFOUND' || err.code === 'EAI_AGAIN') {
                    continue;
                }
                // If it's a different error, log it but continue
                event_logger_1.eventLogger.warning('Ollama', `Unexpected error for ${url}`, err.message);
            }
        }
        event_logger_1.eventLogger.warning('Ollama', 'Connection failed on all URLs. Please ensure Ollama is running: ollama serve');
        event_logger_1.eventLogger.info('Ollama', 'Troubleshooting: Run "curl http://127.0.0.1:11434/api/tags" to verify Ollama is accessible');
        return false;
    }
    catch (error) {
        const errorDetails = {
        ...
        event_logger_1.eventLogger.error('Ollama', 'Connection check failed', JSON.stringify(errorDetails, null, 2));
        return false;
    }
}
async function ensureModelLoaded() {
    ...
    for (const url of urls) {
        try {
            event_logger_1.eventLogger.debug('Ollama', `Checking model availability at ${url}...`);
            const response = await axios_1.default.get(`${url}/api/tags`, {
                timeout: 5000,
                validateStatus: () => true, // Don't throw on HTTP errors
            });
            if (response.status === 200) {
                workingUrl = url;
                event_logger_1.eventLogger.info('Ollama', `Using Ollama at ${url}`);
                break;
            }
        }
        catch (err) {
            event_logger_1.eventLogger.debug('Ollama', `${url} failed: ${err.code || err.message}`);
            if (err.code === 'ECONNREFUSED' || err.code === 'ENOTFOUND' || err.code === 'EAI_AGAIN') {
                continue;
            }
            throw err;
    ...
    try {
        ...
        const response = await axios_1.default.get(`${workingUrl}/api/tags`, {
            timeout: 5000,
            validateStatus: () => true,
        });
        if (response.status !== 200) {
        ...
        const modelExists = models.some((m) => m.name === DEFAULT_MODEL);
        if (!modelExists) {
            ...
            for (const fallback of fallbackModels) {
                const fallbackExists = models.some((m) => m.name === fallback);
                if (fallbackExists) {
                    foundModel = fallback;
                    activeModel = fallback;
                    event_logger_1.eventLogger.info('Ollama', `Using fallback model: ${fallback} (${DEFAULT_MODEL} not found)`);
                    event_logger_1.eventLogger.info('Ollama', `${fallback} is available and will be used for faster inference`);
                    break;
                }
            }
            if (foundModel === DEFAULT_MODEL) {
                event_logger_1.eventLogger.info('Ollama', `Model ${DEFAULT_MODEL} not found, pulling...`);
                event_logger_1.eventLogger.info('Ollama', 'This is the smallest/fastest model (1B params) for RAG tasks');
                await axios_1.default.post(`${workingUrl}/api/pull`, {
                    name: DEFAULT_MODEL,
                    stream: false,
                }, {
                ...
                event_logger_1.eventLogger.success('Ollama', `Model ${DEFAULT_MODEL} loaded and ready`);
            }
        }
        else {
            activeModel = DEFAULT_MODEL;
            event_logger_1.eventLogger.success('Ollama', `Model ${DEFAULT_MODEL} is already available (fastest for RAG - 1B params)`);
        }
    }
    catch (error) {
        event_logger_1.eventLogger.error('Ollama', 'Failed to ensure model is loaded', error.message || error);
...
async function generateAnswer(question, context, pageMetadata) {
...
async function generateAnswerFromPrompt(prompt) {
    ...
    for (const url of urls) {
        try {
            event_logger_1.eventLogger.debug('Ollama', `Attempting generation at ${url}...`);
            const startTime = Date.now();
            ...
            if (response.status === 200 && response.data.response) {
                const duration = Date.now() - startTime;
                event_logger_1.eventLogger.success('Ollama', `Answer generated successfully (${duration}ms)`);
                return { answer: response.data.response, prompt: prompt };
            }
            else {
                event_logger_1.eventLogger.warning('Ollama', `Unexpected response from ${url}`, { status: response.status, data: response.data });
            ...
            lastError = error;
            event_logger_1.eventLogger.debug('Ollama', `${url} failed: ${error.code || error.message}`);
            ...
            if (url === urls[urls.length - 1]) {
                event_logger_1.eventLogger.error('Ollama', 'Generation failed on all URLs', error.message || error);
                throw error;
            }
        }
    }
    // If we get here, all URLs failed
    event_logger_1.eventLogger.error('Ollama', 'Failed to connect to Ollama on any URL', lastError?.message || 'Unknown error');
...
function buildQAPrompt(question, context, pageMetadata) {
 ...
 */
async function generateAnswerWithTools(question, pageContext, availableTools, executeToolCallback) {
        ...
        try {
            ...
            if (toolCallMatch) {
                ...
                try {
                    ...
                    // Execute tool
                    const toolResult = await executeToolCallback(toolCall);
            ...
            event_logger_1.eventLogger.error('Ollama', 'Tool calling failed', error.message || error);
            return {
                success: false,
