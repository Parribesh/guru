...
function buildPrompt(question, context, metadata) {
...
`;
    ...
    for (let i = 0; i < context.primaryChunks.length; i++) {
        ...
        if (chunk.componentType && chunk.componentType !== 'text' && chunkContent.length > MAX_CHARS_PER_CHUNK) {
            event_logger_1.eventLogger.debug('Prompt Builder', `Component chunk ${i + 1} (${chunk.componentType}) is ${chunkContent.length} chars - complete element preserved`);
        }
            ...
            if (chunkContent.length > MAX_CHARS_PER_CHUNK * 1.5) {
                event_logger_1.eventLogger.warning('Prompt Builder', `Text chunk ${i + 1} is ${chunkContent.length} chars (expected max ${MAX_CHARS_PER_CHUNK}). This should have been split during chunking.`);
            }
        }
        ...
        if (totalChars + chunkText.length > availableChars) {
            event_logger_1.eventLogger.warning('Prompt Builder', `Stopping at chunk ${i + 1}/${context.primaryChunks.length} to stay within token limit`);
            break;
        }
    ...
    event_logger_1.eventLogger.info('Prompt Builder', `Prompt size: ~${estimatedTokens} tokens (${finalPrompt.length} chars), using ${limitedChunks.length}/${context.primaryChunks.length} chunks`);
    if (estimatedTokens > MAX_PROMPT_TOKENS) {
        event_logger_1.eventLogger.warning('Prompt Builder', `Prompt may exceed token limit (${estimatedTokens} > ${MAX_PROMPT_TOKENS})`);
    }
    return finalPrompt;
}
