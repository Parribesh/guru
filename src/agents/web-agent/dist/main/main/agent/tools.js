...
async function executeTool(toolCall, browserView) {
    const startTime = Date.now();
    event_logger_1.eventLogger.info('Tool Execution', `Executing tool: ${toolCall.name} with params: ${JSON.stringify(toolCall.params)}`);
    try {
        let result;
        ...
        event_logger_1.eventLogger.success('Tool Execution', `Tool ${toolCall.name} completed in ${duration}ms`);
        return {
        ...
        const duration = Date.now() - startTime;
        event_logger_1.eventLogger.error('Tool Execution', `Tool ${toolCall.name} failed after ${duration}ms`, error.message || error);
        return {
            toolCallId: toolCall.id,
            success: false,
...
async function executeFillInput(browserView, params) {
...
async function executeClickButton(browserView, params) {
    const selector = params.selector;
    if (!selector) {
...
async function executeSelectOption(browserView, params) {
    const selector = params.selector;
    const value = params.value;
    if (!selector || !value) {
...
async function executeSubmitForm(browserView, params) {
    const selector = params.selector;
    if (!selector) {
...
async function executeWaitForElement(browserView, params) {
    const selector = params.selector;
