...
const context_1 = require("../rag/context");
...
async function answerQuestion(request) {
    ...
    event_logger_1.eventLogger.info('QA Service', `💭 Processing your question...`);
    event_logger_1.eventLogger.debug('QA Service', `Question: "${request.question.substring(0, 50)}..."`);
    event_logger_1.eventLogger.debug('QA Service', `Tab ID: ${request.tabId}`);
    try {
        // Step 1: Get context from RAG system
        const contextResult = await (0, context_1.getContextForQuestion)(request.question, request.tabId);
        if (!contextResult) {
            event_logger_1.eventLogger.warning('QA Service', 'No context found for question');
            return {
                success: false,
                answer: '',
                explanation: '',
        ...
        if (!cache) {
            event_logger_1.eventLogger.warning('QA Service', 'No cache found in context result');
            return {
                success: false,
                answer: '',
                explanation: '',
        ...
        // Step 2: Build prompt
        event_logger_1.eventLogger.info('QA Service', `🤖 Building prompt...`);
        const prompt = (0, builder_1.buildPrompt)(request.question, context, {
            url: cache.pageContent.url,
            title: cache.pageContent.title,
        });
        // Step 3: Generate answer using LLM
        event_logger_1.eventLogger.info('QA Service', `🤖 Generating answer...`);
        let answer;
        let finalPrompt;
        try {
            const result = await (0, service_1.generateAnswer)(prompt);
            answer = result.answer;
            finalPrompt = result.prompt;
            event_logger_1.eventLogger.info('QA Service', '✅ Answer generated');
        }
        catch (error) {
            event_logger_1.eventLogger.error('QA Service', 'Answer generation failed', {
            ...
            if (error.message?.includes('Ollama is not running') ||
                ...
                event_logger_1.eventLogger.warning('QA Service', 'Ollama not available, providing fallback response');
                ...
                event_logger_1.eventLogger.error('QA Service', 'Failed to generate answer', error.message || error);
                throw error;
            }
        }
        ...
        event_logger_1.eventLogger.success('QA Service', `Question answered in ${processingTime}ms`);
        return {
            success: true,
            answer,
        ...
        event_logger_1.eventLogger.error('QA Service', 'QA service error', error instanceof Error ? error.message : 'Unknown error');
        return {
            success: false,
            answer: '',
            explanation: '',
