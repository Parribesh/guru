...
const ViewService_1 = require("../windows/ViewService");
const bounds_1 = require("../windows/bounds");
...
class TabManager {
    ...
    getDevDefaultUrl() {
        ...
        if (__dirname.includes('dist')) {
            // Compiled: dist/main/main/ -> go up to project root, then test-fixtures/
            const projectRoot = path.resolve(__dirname, '../../../../');
            devSamplePath = path.join(projectRoot, 'test-fixtures', 'dev-sample.html');
        }
        else {
            // Source: app/main/ -> go up to project root, then test-fixtures/
            devSamplePath = path.join(__dirname, '../../../test-fixtures/dev-sample.html');
        }
        ...
        if (!fs.existsSync(normalizedPath)) {
            ...
            const fallbackPath = path.join(process.cwd(), 'test-fixtures', 'dev-sample.html');
    ...
    async createTab(url) {
        ...
        const view = await ViewService_1.ViewService.createBrowserView(tab, this.preloadPath, this.mainWindow);
    ...
    switchToTab(tabId) {
        ...
        if (newView) {
            ...
            (0, bounds_1.setBrowserViewBounds)(newView, this.mainWindow);
    ...
    navigate(tabId, url) {
        const view = this.views.get(tabId);
        if (!view) {
            event_logger_1.eventLogger.error('Navigation', `Cannot navigate: View not found for tab ${tabId}`);
            return false;
        }
        ...
        event_logger_1.eventLogger.info('Navigation', `Fetching article: ${fullUrl}`);
        event_logger_1.eventLogger.info('Navigation', `Tab ID: ${tabId}`);
    ...
    getTabIdByWebContents(webContentsId) {
    ...
    onWindowResize(width, height) {
        if (this.activeTabId) {
            ...
            if (view) {
                (0, bounds_1.setBrowserViewBounds)(view, this.mainWindow);
            }
        }
    }
}
