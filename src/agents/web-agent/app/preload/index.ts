...
const electronAPI = {
  ...
  on: (channel: string, callback: (...args: any[]) => void) => {
    ...
    if (allowedChannels.includes(channel)) {
      ...
      const wrappedCallback = (ipcEvent: any, ...args: any[]) => {
  ...
  jobs: {
    list: (limit?: number, status?: string) =>
      ipcRenderer.invoke(IPCChannels.jobs.list, limit, status),
    get: (jobId: string) =>
      ipcRenderer.invoke(IPCChannels.jobs.get, jobId),
    count: (status?: string) =>
      ipcRenderer.invoke(IPCChannels.jobs.count, status),
    delete: (jobId: string) =>
      ipcRenderer.invoke(IPCChannels.jobs.delete, jobId),
  },

  // Queue API
  queue: {
    getStatus: () => ipcRenderer.invoke('queue:status'),
    getMetrics: () => ipcRenderer.invoke('queue:metrics'),
  },
